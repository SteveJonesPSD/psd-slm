...
async function getApiUser(supabase: Awaited<ReturnType<typeof createClient>>): Promise<ApiUserWithPrefs | null> {
  ...
  const { data: appUser } = await supabase
    .from('users')
    .select('id, org_id, email, first_name, last_name, initials, color, avatar_url, theme_preference, must_change_password, role_id, ai_preferences, roles(id, name, display_name)')
