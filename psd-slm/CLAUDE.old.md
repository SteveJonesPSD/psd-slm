...
### Per-User Mail Credentials
- **Migration:** `20260401000001_user_mail_credentials.sql` — `user_mail_credentials` table (per-user OAuth tokens), `quote_email_sends` table (send tracking)
- **Table:** `user_mail_credentials` — stores `access_token`, `refresh_token`, `token_expires_at`, `email_address`, `display_name` per user per org. Unique constraint on `(org_id, user_id)`.
- **RLS:** Users can read their own credential; admins can manage all.
- **Types:** `UserMailCredential`, `QuoteEmailSend` in `lib/email/types.ts`

### UserGraphClient vs GraphClient
- **`GraphClient`** (`lib/email/graph-client.ts`) — uses application-level client credentials (`grant_type=client_credentials`). Sends as a shared mailbox via `/users/{mailbox}/sendMail`. Used for helpdesk email.
- **`UserGraphClient`** (`lib/email/user-graph-client.ts`) — uses per-user delegated OAuth (`grant_type=refresh_token`). Sends via `/me/sendMail` so the email appears in the user's own Outlook Sent Items. Used for quote sending.
- **Key difference:** `saveToSentItems: true` is mandatory on all sales sends — this is why per-user delegated auth was chosen.
- **Token refresh:** `UserGraphClient.getToken()` checks stored token expiry (5 min buffer), refreshes via `refresh_token` grant if needed, updates DB. On permanent auth failure (400/401), marks credential as inactive.

### OAuth Flow
- **Initiation:** `GET /api/auth/mail-connect?user_id={id}` — builds Microsoft OAuth2 URL with `Mail.Send + User.Read + offline_access` delegated scopes. CSRF state stored in `org_settings`.
- **Callback:** `GET /api/auth/mail-callback` — exchanges code for tokens, fetches `/me` for email/display name, upserts `user_mail_credentials`. Redirects to `/team`.
- **Azure AD requirements:** App Registration needs delegated `Mail.Send`, `User.Read`, `offline_access` permissions. Redirect URI: `{SITE_URL}/api/auth/mail-callback`.

### Send Quote Action
- **Server action:** `sendQuoteEmail()` in `quotes/send-actions.ts`
- **Flow:** Load quote → load sender credential → generate PDF (if needed) → build branded HTML email → send via `UserGraphClient` → record in `quote_email_sends` → update quote status to `sent` (first send only) → log activity
- **PDF generation:** Uses `@react-pdf/renderer` server-side with `QuotePdfDocument` component, base64-encoded as Graph attachment
- **Custom headers:** `X-Engage-Quote-ID` and `X-Engage-Quote-Number` stored for future NDR matching
- **Fallback:** If assigned user has no connected mailbox, modal allows selecting any org user with credentials

### Send Quote Modal
- **File:** `quotes/[id]/send-quote-modal.tsx` — 2-step modal
- **Step 1:** Choose send method — PDF attachment, portal link, or both
- **Step 2:** Compose — From (auto-filled from sender cred), To (pre-filled from contact), Subject, Message body, preview section
- **Resend:** Same modal with `isResend` prop — subject prefixed "Resend:", status unchanged

### Portal Enhancements
- **PDF download:** `PortalPdfButton` component on portal page. PDF route (`/api/quotes/[id]/pdf`) supports `?token=` query param for unauthenticated portal access.
- **Group headers:** Portal line items table shows group section headers when multiple groups exist.
- **Brand footer:** Brand contact details and footer text shown at bottom of portal view.

### Team Page Integration
- **"Email Sending" column** on team table — shows Connected/Not connected badge with email address
- **"Connect Mailbox" / "Disconnect" buttons** in actions column (admin only)
- Mail credentials fetched via admin client and passed to `TeamTable` component
